...
class DataParser {
  ...
  Future<DataHandler> fetchCategories(pid) async {
    ...
    if (pid == 0) {
      String strUrl = sprintf(
          "%s?api_key=%s", [Config.GET_CATEGORIES_JSON_URL, Config.API_KEY]);
      print(strUrl);
      ...
      dataHandler.categories =
          await DBProvider.instance.getCategoriesByPID(pid);
    }
    return dataHandler;
  }
  ...
  Future<DataHandler> getContent(strUrl, isMap) async {
    ...
    dataHandler.categories = <Category>[];
    dataHandler.news = <News>[];
    dataHandler.nearbyStores = <Store>[];
    dataHandler.stores = <Store>[];
    dataHandler.topRated = <Store>[];
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonData != null) dataHandler.nearbyStores = getStores(jsonData);
            ...
            for (Store store in dataHandler.stores!) {
              if (store.mapIcon.length > 0) {
                store.mapIconDescriptor =
                    await MapHelpers.fetchMapIcon(store.mapIcon);
              }
            }
          }
        }
        ...
        if (jsonData != null) dataHandler.topRated = getStores(jsonData);
      }
    } catch (e) {
      print('(TRACE) = ' + e.toString());
    }
  ...
  Future<DataHandler> getSavedStores() async {
    ...
    for (Store store in stores) {
      store.photos =
          await DBProvider.instance.getPhotosByStoreId(store.storeId);
    }
  ...
  Future<DataHandler> fetchCategoryStoresNearby(Category category) async {
    ...
    if (connectivityResult == ConnectivityResult.none) {
      DataHandler dataHandler = new DataHandler();
      dataHandler.stores =
          await DBProvider.instance.getStoresByCategoryId(category.categoryId);
      return dataHandler;
    }
    ...
    double radius = GetIt.instance.get<SessionStorage>().getRadius;
    String strUrl =
        sprintf("%s?api_key=%s&lat=%f&lon=%f&radius=%f&category_id=%d", [
      Config.GET_STORES_JSON_URL,
      Config.API_KEY,
      position!.latitude,
      position.longitude,
  ...
  Future<DataHandler> login(String username, String password) async {
    ...
    if (connectivityResult == ConnectivityResult.mobile ||
      ...
      try {
        var response =
            await http.post(Uri.parse(Config.LOGIN_URL), body: params);
        if (response.statusCode == 200) {
          var dataJson = json.decode(response.body);
          print(dataJson);
  ...
  Future<DataHandler> register(username, password, email, fullName, facebookId,
      twitterId, appleId, thumbUrl, thumbFile) async {
    DataHandler dataHandler = DataHandler();
    var connectivityResult = await (Connectivity().checkConnectivity());
    if (connectivityResult == ConnectivityResult.mobile ||
        connectivityResult == ConnectivityResult.wifi) {
      try {
        var request =
            new http.MultipartRequest("POST", Uri.parse(Config.REGISTER_URL));
        ...
        if (thumbFile != null)
          request.files
              .add(await http.MultipartFile.fromPath('thumb_file', thumbFile));
  ...
  Future<DataHandler> updateProfile(
      userId, password, fullName, thumbUrl, thumbFile) async {
    DataHandler dataHandler = DataHandler();
    var connectivityResult = await (Connectivity().checkConnectivity());
    if (connectivityResult == ConnectivityResult.mobile ||
        connectivityResult == ConnectivityResult.wifi) {
      try {
        var request = new http.MultipartRequest(
            "POST", Uri.parse(Config.UPDATE_USER_PROFILE_URL));
  ...
  Future<DataHandler> getRatingStore(
      {int? userId, String? loginHash, int? storeId}) async {
    Map<String, String?> params = {
    ...
    if (connectivityResult == ConnectivityResult.mobile ||
        connectivityResult == ConnectivityResult.wifi) {
      try {
        var response = await http.post(Uri.parse(Config.GET_RATING_USER_URL),
            body: params);
        if (response.statusCode == 200) {
          var dataJson = json.decode(response.body);
          print(dataJson);
  ...
  Future<DataHandler?> postRating(
      {int? userId, String? loginHash, int? storeId, double? rating}) async {
    Map<String, String?> params = {
      "user_id": userId.toString(),
      "login_hash": loginHash,
      "store_id": storeId.toString(),
    ...
    if (connectivityResult == ConnectivityResult.mobile ||
      ...
      try {
        var response =
            await http.post(Uri.parse(Config.POST_RATING_URL), body: params);
        if (response.statusCode == 200) {
          var dataJson = json.decode(response.body);
  ...
  Future<DataHandler?> getReviews({int? min, int? max, int? storeId}) async {
    String strUrl = sprintf("%s?api_key=%s&min=%d&max=%d&store_id=%d",
        [Config.GET_REVIEWS_JSON_URL, Config.API_KEY, min, max, storeId]);

    DataHandler dataHandler = DataHandler();
    dataHandler.reviews = <Review>[];
    var connectivityResult = await (Connectivity().checkConnectivity());
    if (connectivityResult == ConnectivityResult.mobile ||
        connectivityResult == ConnectivityResult.wifi) {
      try {
        ...
        if (response.statusCode == 200) {
          ...
          dataHandler.totalRowCount =
              int.parse(dataJson['total_row_count'].toString());
  ...
  Future<DataHandler?> postReview(
      {int? userId, String? loginHash, int? storeId, String? review}) async {
    Map<String, String?> params = {
      "user_id": userId.toString(),
      "login_hash": loginHash,
      "store_id": storeId.toString(),
    ...
    if (connectivityResult == ConnectivityResult.mobile ||
      ...
      try {
        var response =
            await http.post(Uri.parse(Config.POST_REVIEW_URL), body: params);
        if (response.statusCode == 200) {
          var dataJson = json.decode(response.body);
          var jsonRating = dataJson['status'];
          if (jsonRating != null && jsonRating['status_code'] == "-1") {
  ...
  Future<DataHandler> insertStore(
    ...
    if (connectivityResult == ConnectivityResult.mobile ||
      ...
      try {
        var request =
            new http.MultipartRequest("POST", Uri.parse(Config.POST_STORE_URL));
        ...
        for (int x = 0; x < photoPaths!.length; x++) {
          if (photoPaths[x].runtimeType == String) {
            ...
            request.files
                .add(await http.MultipartFile.fromPath(strPath, photoPaths[x]));
            ind += 1;
          }
        ...
        for (int x = 0; x < photoIdsDeleted!.length; x++) {
          ...
          if (x < photoIdsDeleted.length - 1) ids += ",";
        }
  ...
  Future<DataHandler> fetchNews({int? min, int? max}) async {
    ...
    String strUrl = sprintf("%s?api_key=%s&min=%d&max=%d",
        [Config.GET_NEWS_JSON_URL, Config.API_KEY, min, max]);
    print(strUrl);
  ...
  Future<DataHandler> searchStores(
      {int? min, int? max, String? searchStr}) async {
    Position? position = await MyApp.determinePosition();
    print(position);
    ...
    if (connectivityResult == ConnectivityResult.none) {
      ...
      var response =
          await http.post(Uri.parse(Config.SEARCH_STORES_URL), body: params);
      if (response.statusCode == 200) {
        var dataJson = json.decode(response.body);
        print(dataJson);
  ...
  Future<bool> postPlayerId({String? playerId}) async {
    ...
    if (connectivityResult == ConnectivityResult.mobile ||
      ...
      try {
        var response =
            await http.post(Uri.parse(Config.POST_PLAYER_ID_URL), body: params);
        if (response.statusCode == 200) {
          var dataJson = json.decode(response.body);
          var jsonRating = dataJson['status'];
          if (jsonRating != null && jsonRating['status_code'] == "-1") {
  ...
  Future<DataHandler> deleteStore({storeId, userId, loginHash}) async {
    DataHandler dataHandler = DataHandler();
    var connectivityResult = await (Connectivity().checkConnectivity());
    if (connectivityResult == ConnectivityResult.mobile ||
        connectivityResult == ConnectivityResult.wifi) {
      try {
        var request = new http.MultipartRequest(
            "POST", Uri.parse(Config.DELETE_STORE_URL));
        request.fields['store_id'] = storeId;
        request.fields['login_hash'] = loginHash;
        request.fields['user_id'] = userId.toString();
        request.fields['api_key'] = Config.API_KEY;
