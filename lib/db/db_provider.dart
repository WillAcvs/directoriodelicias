...
class DBProvider {
  ...
  deleteStore(int id) async {
    final db = await database;
    var res =
        await db!.delete("stores", where: "store_id = ?", whereArgs: [id]);
    return res;
  }
  ...
  deletePhoto(int id) async {
    final db = await database;
    var res =
        await db!.delete("photos", where: "photo_id = ?", whereArgs: [id]);
    return res;
  }
  ...
  deletePhotosByStoreId(int id) async {
    final db = await database;
    var res =
        await db!.delete("photos", where: "store_id = ?", whereArgs: [id]);
    return res;
  }
  ...
  deleteCategory(int id) async {
    final db = await database;
    var res = await db!
        .delete("categories", where: "category_id = ?", whereArgs: [id]);
    return res;
  }
  ...
  Future<List<Fave?>> getFaves() async {
    ...
    List<Fave> objs =
        res.isNotEmpty ? res.map((c) => Fave.fromJson(c)).toList() : [];
    return objs;
  }
  ...
  Future<List<Store>> getStoresFave() async {
    final db = await database;
    String sql =
        "SELECT * FROM stores JOIN faves ON stores.store_id = faves.store_id ORDER BY distance ASC";
    var res = await db!.rawQuery(sql);
    List<Store> objs =
        res.isNotEmpty ? res.map((c) => Store.fromJsonRaw(c)).toList() : [];
  ...
  Future<List<Notification>> getNotifications() async {
    ...
    var res = await db!.rawQuery(sql);
    List<Notification> objs = res.isNotEmpty
        ? res.map((c) => Notification.fromJsonRaw(c)).toList()
        : [];
    print(res.toString());
    return objs;
  }
  ...
  Future<List<Photo>> getPhotosByStoreId(int id) async {
    ...
    var res = await db!.rawQuery(sql);
    List<Photo> objs =
        res.isNotEmpty ? res.map((c) => Photo.fromJson(c)).toList() : [];
    return objs;
  }
  ...
  Future<User?> getLoggedUser() async {
    ...
    var res = await db!.rawQuery(sql);
    List<User> objs =
        res.isNotEmpty ? res.map((c) => User.fromJsonRaw(c)).toList() : [];
    return objs.isNotEmpty ? objs.first : null;
  }
  ...
  Future<List<Category>> getCategories() async {
    ...
    var res = await db!.rawQuery(sql);
    List<Category> objs =
        res.isNotEmpty ? res.map((c) => Category.fromJsonRaw(c)).toList() : [];
    return objs;
  }
  ...
  Future<List<Category>> getCategoriesLeafNode() async {
    final db = await database;
    String sql =
        "SELECT t1.category_id, t1.category, t1.created_at, t1.updated_at, t1.is_deleted, t1.pid FROM categories AS t1 LEFT JOIN categories as t2 ON t1.category_id = t2.pid WHERE t2.category_id IS NULL AND t1.is_deleted = 0 AND t1.pid >= 0 ORDER BY t1.category ASC";
    var res = await db!.rawQuery(sql);
    List<Category> objs =
        res.isNotEmpty ? res.map((c) => Category.fromJsonRaw(c)).toList() : [];
    return objs;
  ...
  Future<List<Category>> getCategoriesByPID(pid) async {
    final db = await database;
    String sql = sprintf(
        "SELECT * FROM categories WHERE pid = %d ORDER BY category ASC", [pid]);
    var res = await db!.rawQuery(sql);
    List<Category> objs =
        res.isNotEmpty ? res.map((c) => Category.fromJsonRaw(c)).toList() : [];
    return objs;
  ...
  Future<List<Store>> getFeatured() async {
    final db = await database;
    String sql =
        "SELECT * FROM stores WHERE featured = 1 ORDER BY distance ASC";
    var res = await db!.rawQuery(sql);
    List<Store> objs =
        res.isNotEmpty ? res.map((c) => Store.fromJsonRaw(c)).toList() : [];
  ...
  Future<List<Store>> getStoresByCategoryId(id) async {
    final db = await database;
    String sql = sprintf(
        "SELECT * FROM stores WHERE category_id = %d ORDER BY distance ASC",
        [id]);
    var res = await db!.rawQuery(sql);
    List<Store> objs =
        res.isNotEmpty ? res.map((c) => Store.fromJsonRaw(c)).toList() : [];
